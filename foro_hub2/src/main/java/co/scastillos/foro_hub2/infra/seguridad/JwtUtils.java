...
public class JwtUtils {
    ...
    private String autorToken;


    public String crearToken(Authentication authentication){

            Algorithm algorithm = Algorithm.HMAC256(this.llavePrivada);

            String nombre = authentication.getName();
            String autorizaciones = authentication.getAuthorities()
                    .stream().map(GrantedAuthority::getAuthority)
                    .collect(Collectors.joining(","));

            String jwtToken = JWT.create()
                    .withIssuer(this.autorToken)
                    .withSubject(nombre)
                    .withClaim("autorizaciones", autorizaciones)
