   ...
   private final JwtUtils jwtUtils;
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        if(jwtToken != null){ //&& jwtToken.startsWith("Bearer ")
            ...
            String  nombre = jwtUtils.extractUsername(decodedJWT);
            String stringAuthorities = jwtUtils.getSpecificClaim(decodedJWT,"autorizaciones").asString();

            Collection<? extends GrantedAuthority> authorities = AuthorityUtils.commaSeparatedStringToAuthorityList(stringAuthorities);

            SecurityContext contex = SecurityContextHolder.getContext();
            Authentication authentication = new UsernamePasswordAuthenticationToken(nombre,null,authorities);
